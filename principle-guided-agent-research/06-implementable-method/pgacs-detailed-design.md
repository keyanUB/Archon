...
Each event gets exactly one primary process label and zero or more secondary attribute labels.
...
2. **Selection** → produce `PolicyState v0` (compact set).
3. **Distribution** → materialize each policy's per-phase bindings.
4. **Event ingestion** → route every `ObservationEvent` to Layer B monitors and
   to `reassess()`.
5. **Dynamic adoption** → compute `PolicyDelta`s; tighten `PolicyState`
   (monotonic).
6. **Loop conditioning** → run the predict-then-condition step at repair
   boundaries (Layer C).
7. **Evidence ledger** → append observed evidence; never accept model prose as
   evidence.
8. **Gate** → compute the deterministic `TerminalStatus`.
...
interface PolicyController {
  state: PolicyState;
  select(surface: TaskSurface): PolicyState; // v0
...
7. Validate      run required probes/tests per active policies (Layer B active tier).
8. Gate          controller.gate() reads ledger → TerminalStatus.
9. Report        final report generated FROM the ledger, not from model prose.
```

### 8.2 Degraded sequence (log/diff adapter, `inject:false, interceptTools:false`)

```text
Surface → Select → (no live injection: guidance front-loaded once if any inject
path exists, else none) → agent runs freely → controller consumes the final
diff + logs as ObservationEvents → Layer B monitors + active probes run post-hoc
→ Gate → Report. Enforcement = detect + verify + re-prompt loop; run metadata
records "enforcement: detective-only".
```
...
```text
if any fail_closed policy status == fail        → FAILED_POLICY_VALIDATION
...
2. One adapter on Archon (context + hook/broker → inject + interceptTools).
...
4. Deterministic controller: a few dynamic triggers + monotonic hardening.
5. One active probe + one critic-LLM check (prove the expensive path).

**Then widen:** more adapters (Codex driver, raw-Claude-Code hook, log/diff);
more packs (web-api, database, env-setup); the learned ranker; the first
non-code-generation pack.

**Milestones (gate on these):**

- **M1** — On 5–10 known-risky tasks, the harness fills an evidence ledger and
  blocks ≥1 real unsafe shortcut a baseline agent takes.
