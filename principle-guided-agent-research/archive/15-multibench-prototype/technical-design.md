...
Scope boundary: this specification controls whole-artifact BaxBench generation
where `app.py` is the candidate. Its adapter interfaces are reusable, but its
candidate, workspace, context, scope, and repair details are not the normative
design for masked repository completion. The separately versioned SecRepoBench
specialization is specified in
