...
cases. SecRepoBench is intentionally outside this frozen v0.3 registry and
experiment. It is now pursued as a separately versioned specialization because
masked repository completion requires a different candidate envelope,
history-free repository materialization, bounded repository surface extraction,
region integrity, and ARVO/OSS-Fuzz evaluation. That design lives in
`principle-guided-agent-research/current-secrepobench/technical-design.md`; it does not retroactively
change the v0.3 implementation or results.
...
- Treat SecRepoBench as a task-form specialization, not another manifest for the
  BaxBench runner. Require its patch candidate, repository leakage boundary,
  context extractor, and region-integrity controls before evaluating adapter
  generality.
